  ...
  await fetch(`${process.env.NEXT_PUBLIC_BACKEND_URL}/addEvent/?user=${User}`, {
    ...
    return (
        <div onClick={() => setAddFormToggle(false)} className="fixed bottom-0 left-0 w-full h-full bg-black/50 z-1 border border-amber-50">
            <form onClick={(e) => e.stopPropagation() } action={formSubmit} className="bg-foreground fixed top-1/2 left-1/2 -translate-x-1/2 -translate-y-1/2 z-2 flex flex-col gap-4 justify-center items-center max-w-[500px] w-10/12 h-contain px-sm py-md rounded-2xl shadow-lg shadow-gray-700">
                <motion.input type="text" name="title" autoFocus whileHover={{scale: 1.05}} placeholder="Task" className="w-full h-1/6 p-2 bg-(--accent-orange) border-(--cta) border-b text-(--bg-primary) rounded-md shadow-md shadow-gray-400 focus:bg-foreground focus:outline-none focus:border-(--cta) focus:border-b-2 focus:rounded-none focus:text-black " />
